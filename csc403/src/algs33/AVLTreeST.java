...
public class AVLTreeST<Key extends Comparable<Key>, Value> {
    ...
    private Node balance(Node node) {
        if (balanceFactor(node) < -1) {
            ...
            // Otherwise was inserted to the right of the right child
            ...
            if (balanceFactor(node.left) < 0) {
            	...
            	// The new node was inserted to the right of the left child
                node.left = rotateLeft(node.left);
            }
            
            // Otherwise was inserted to the left of the left child
